...
-- CHALLENGE QUESTION
SELECT pr1.specialty_description, COUNT(drug_name) AS drug_count
FROM prescriber AS pr1 FULL JOIN prescription AS pr2 USING (npi)
GROUP BY pr1.specialty_description 
ORDER BY drug_count ASC

-- BONUS - NOT COMPLETED
SELECT DISTINCT(pr2.specialty_description), pr1.bene_count
FROM prescription AS pr1 FULL JOIN prescriber AS pr2 USING(npi)
				  		 FULL JOIN drug USING (drug_name)
WHERE drug.opioid_drug_flag = 'Y'
GROUP BY pr2.specialty_description, pr1.bene_count


		(SELECT npi,total_claim_count
		FROM prescription
		GROUP BY npi, total_claim_count
		ORDER BY total_claim_count)) AS percentage
FROM prescription AS pr1 FULL JOIN prescriber AS pr2 USING(npi)
				  		 FULL JOIN drug USING (drug_name)
...
SELECT npi,(npi-1000) AS difference
FROM prescriber
GROUP BY npi
...
/* Number 6. */
-- A. 
WITH total_claims AS /* Run A from SELECT on Line 102 */
					(SELECT npi, drug_name, total_claim_count 
					 FROM prescription
					 WHERE total_claim_count > 3000)
-- B and C. Run A, B, and C together.		 
...
/* Number 7 */
-- A. 
SELECT pr.npi, pr.specialty_description, pr.nppes_provider_city, dr.drug_name, dr.opioid_drug_flag
FROM prescriber AS pr
CROSS JOIN drug AS dr
WHERE pr.specialty_description LIKE 'Pain Management' AND pr.nppes_provider_city ILIKE 'Nashville' AND dr.opioid_drug_flag LIKE 'Y';

SELECT *
FROM drug

-- B.
SELECT pr1.npi, drug.drug_name, pr2.total_claim_count
FROM prescriber AS pr1
CROSS JOIN drug FULL JOIN prescription AS pr2 USING (drug_name, npi)
WHERE pr1.specialty_description LIKE 'Pain Management' AND pr1.nppes_provider_city ILIKE 'Nashville' AND drug.opioid_drug_flag LIKE 'Y'
GROUP BY pr1.npi, drug.drug_name, pr2.total_claim_count;

-- C. 
SELECT pr1.npi, drug.drug_name, COALESCE(pr2.total_claim_count::text,'none') AS total_claim_count
FROM prescriber AS pr1
CROSS JOIN drug FULL JOIN prescription AS pr2 USING (drug_name, npi)
WHERE pr1.specialty_description LIKE 'Pain Management' AND pr1.nppes_provider_city ILIKE 'Nashville' AND drug.opioid_drug_flag LIKE 'Y'
GROUP BY pr1.npi, drug.drug_name, pr2.total_claim_count;	 
